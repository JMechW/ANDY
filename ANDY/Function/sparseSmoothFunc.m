function [out0,out1]=sparseSmoothFunc(order,xx,xxbb,xxAlpha,xxrr)
% Author: Jiamin Wang; Updated: 2021-12-15;

%     [out0,out1]=sparseSmoothFunc(order,sigma,magma,xx,xxbb,xxAlpha,xxrr)
    if order<=0
    ...
    medTerm2=medTerm1.^(order/2);
    
    out0 = medTerm2 - xxrr.^order;
    out1 = (order).*(xxAlpha).*scaledDifference.*(medTerm2./medTerm1);
